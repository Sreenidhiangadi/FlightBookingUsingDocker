...
class BookingControllerTest {
	...
	void bookTicket_shouldCallServiceAndReturnPnrWrappedInResponseEntity() {
		...
		when(bookingService.bookTicket("sreenidhi@gmail.com", "DEP123", "RET123", request.getPassengers(),
				FLIGHTTYPE.ROUND_TRIP)).thenReturn(Mono.just("PNR123"));

		webTestClient.post().uri("/api/flight/booking/{flightId}", "DEP123").contentType(MediaType.APPLICATION_JSON)
				.bodyValue(request).exchange().expectStatus().isOk().expectBody(String.class).isEqualTo("PNR: PNR123");
		...
		verify(bookingService).bookTicket(emailCaptor.capture(), depIdCaptor.capture(), retIdCaptor.capture(),
				passengersCaptor.capture(), typeCaptor.capture());
	...
	void getTicket_shouldReturnTicketFromService() {
		...
		webTestClient.get().uri("/api/flight/ticket/{pnr}", "PNR123").exchange().expectStatus().isOk().expectBody()
				.jsonPath("$.pnr").isEqualTo("PNR123").jsonPath("$.userEmail").isEqualTo("sreenidhi@gmail.com");
	...
	void history_shouldReturnFluxOfTickets() {
		...
		when(bookingService.historyByEmail("sreenidhi@gmail.com")).thenReturn(Flux.just(ticket1, ticket2));

		webTestClient.get().uri("/api/flight/booking/history/{emailId}", "sreenidhi@gmail.com").exchange()
				.expectStatus().isOk().expectBodyList(Ticket.class).value(tickets -> {
	...
	void cancel_shouldDelegateToServiceAndReturnResult() {
		...
		webTestClient.delete().uri("/api/flight/booking/cancel/{pnr}", "PNR123").exchange().expectStatus().isOk()
				.expectBody(String.class).isEqualTo("Cancelled");
