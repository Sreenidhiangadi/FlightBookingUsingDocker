...
class FlightServiceImplTest {
    ...
    void searchFlightById_whenNotPresent_returnsError() {
        ...
        StepVerifier.create(flightService.searchFlightById("F1"))
                .expectErrorMatches(ex -> ex.getMessage().contains("Flight with this ID is not present"))
                .verify();
    }
    ...
    void reserveSeats_successfulReservation() {
        ...
        StepVerifier.create(flightService.reserveSeats("F1", 10))
                .expectNextMatches(f -> f.getAvailableSeats() == 40)
                .verifyComplete();
    }
    ...
    void releaseSeats_shouldIncreaseSeatCount() {
        ...
        StepVerifier.create(flightService.releaseSeats("F1", 5))
                .expectNextMatches(f -> f.getAvailableSeats() == 55)
                .verifyComplete();
    }
    ...
    void searchFlights_shouldDelegateToRepository() {
        ...
        StepVerifier.create(flightService.searchFlights("BLR", "DEL", start, end))
                .expectNext(flight)
                .verifyComplete();
    }
    ...
    void searchFlightsByAirline_shouldDelegateToRepository() {
        when(flightRepository.findByFromPlaceAndToPlaceAndAirline(
                "BLR", "DEL", "Indigo"))
                .thenReturn(Flux.just(flight));

        StepVerifier.create(flightService.searchFlightsByAirline("BLR", "DEL", "Indigo"))
                .expectNext(flight)
                .verifyComplete();
    }
}
