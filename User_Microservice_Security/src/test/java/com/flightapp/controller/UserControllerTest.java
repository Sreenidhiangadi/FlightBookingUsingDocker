...
class UserControllerTest {
	...
	void register_shouldReturnCreatedMessageWithUserId() {
		...
		webTestClient.post().uri("/api/user/register").contentType(MediaType.APPLICATION_JSON).bodyValue(requestUser)
				.exchange().expectStatus().isCreated().expectBody(String.class).isEqualTo("user created with id: 123");
	...
	void login_shouldDelegateToAuthServiceAndReturnToken() {
		...
		when(authService.login(eq("sreenidhi@gmail.com"), eq("password123"))).thenReturn(Mono.just("fake-jwt-token"));

		webTestClient.post().uri("/api/user/login").contentType(MediaType.APPLICATION_JSON).bodyValue(loginUser)
				.exchange().expectStatus().isOk().expectBody(String.class).isEqualTo("fake-jwt-token");
	...
	void getByEmail_shouldReturnUserFromService() {
		...
		webTestClient.get().uri("/api/user/{email}", email).exchange().expectStatus().isOk().expectBody()
				.jsonPath("$.id").isEqualTo("42").jsonPath("$.email").isEqualTo(email);
