...
class AuthServiceImplTest {
	...
	void register_whenUserDoesNotExist_shouldSaveUser() {
		...
		when(userRepository.findByEmail("sreenidhi@gmail.com")).thenReturn(Mono.empty());
		when(userRepository.save(inputUser)).thenReturn(Mono.just(inputUser));

		StepVerifier.create(authService.register(inputUser)).expectNext(inputUser).verifyComplete();
	}
	...
	void login_whenUserExistsAndPasswordMatches_shouldReturnSessionId() {
		...
		StepVerifier.create(authService.login("sreenidhi@gmail.com", "secret")).assertNext(sessionId -> {
	...
	void login_whenUserNotFound_shouldError() {
		...
		StepVerifier.create(authService.login("unknown@example.com", "whatever")).expectErrorSatisfies(ex -> {
	...
	void login_whenPasswordInvalid_shouldError() {
		...
		StepVerifier.create(authService.login("sreenidhi@gmail.com", "wrong")).expectErrorSatisfies(ex -> {
			assertEquals(RuntimeException.class, ex.getClass());
